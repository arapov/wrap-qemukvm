...
# CONFIG:
HOME='/home/anton'
LIBVIRT='/home/anton/media/libvirt'
...
ROOT_UUID=a34756a3-98ce-4c78-b233-df5481e96022
...
while getopts "n:e:k:i:s" opt; do
	case $opt in
		n)
			ip link show $OPTARG >/dev/null 2>&1
			if [ $? -ne 0 ]; then
				echo "[ERROR] Network interface doesn't exist!" >&2
				ip link
				exit 1
			fi
		...
		e)
			...
			if [ -d $OPTARG ]; then
				...
				echo "[ERROR] Exported path is not a directory!" >&2
				exit 1
			fi
			;;
		k)
		...
		s)
			...
			KERNEL=$LIBVIRT/kernels/vmlinuz-3.4.0-1.fc17.x86_64
			INITRD=$LIBVIRT/kernels/initramfs-3.4.0-1.fc17.x86_64.img
			;;
		\?)
...
if [ -s "$KERNEL" ]; then
	...
	# BOOT PARAMETERS
	CLI_BOOT="-append 'root=UUID=$ROOT_UUID ro rd.md=0 rd.lvm=0 rd.dm=0 SYSFONT=True KEYTABLE=us rd.luks=0 LANG=en_US.UTF-8 console=ttyS0 vga=773 3'"
	...
	echo "[INFO] KVM start"
	sudo su -c "qemu-kvm -M pc-0.15 -enable-kvm -rtc base=utc -m 2048 -kernel $KERNEL -initrd $INITRD \
				$CLI_CPU \
				$CLI_DEFAULTS \
				$CLI_EXPORT \
				$CLI_HDD \
